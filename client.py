...
def get_server_public_key():
    public_key = (65537, 3233)  # Example public key (replace with actual key)
    return public_key

def encrypt_des_key(des_key, public_key):
    des_key_hex = hex(des_key)[2:]  # Convert integer to hexadecimal string (remove '0x' prefix)
    return RSA.encrypt(des_key_hex, public_key)

def main():
    host = 'localhost'
    port = 8080  # Changed port number
    ...
    try:
        ...
        des_key = 17336  # Example integer DES key
        encrypted_des_key = encrypt_des_key(des_key, server_public_key)
        print(f"Encrypted DES key: {encrypted_des_key}")

        plaintext = input("Enter the plaintext: ")

        message = f"{','.join(map(str, encrypted_des_key))},{plaintext}"
